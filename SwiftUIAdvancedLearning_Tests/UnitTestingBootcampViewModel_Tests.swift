...
final class UnitTestingBootcampViewModel_Tests: XCTestCase {
    ...
    func test_UnitTestingBootcampViewModel_dataArray_shouldAddItems() {
        ...
        for _ in 0..<loopCount {
            vm.addItem(item: UUID().uuidString) // <- use a random string (better to create your own function with character count as input)
        }
        // Then
        XCTAssertTrue(!vm.dataArray.isEmpty)
        ...
        // XCTAssertGreaterThanOrEqual, ACTAssertLesThan, ACTAssertLesThanOrEqual
    }
    ...
    func test_UnitTestingBootcampViewModel_selectedItem_shouldBeNilWhenSelectingInvaildItem() {
        //Given
        let vm = UnitTestingBootcampViewModel(isPremium: Bool.random())
        // When
        // select valid item
    ...
    func test_UnitTestingBootcampViewModel_saveItem_shouldThrowError_itemNotFound() {
        ...
        // Then
        XCTAssertThrowsError(try vm.saveItem(item: UUID().uuidString)) // <- should throw item not found because this item is not in array
    ...
    func test_UnitTestingBootcampViewModel_downloadWithEscaping_shouldReturnItems() {
        ...
        vm.$dataArray
            .dropFirst() // omit the first blank array
    ...
    func test_UnitTestingBootcampViewModel_downloadWithCombine_shouldReturnItems() {
        ...
        vm.$dataArray
            .dropFirst() // omit the initial value blank array
            .sink { returnedItems in
                expectation.fulfill()
            }
            .store(in: &cancellables)
